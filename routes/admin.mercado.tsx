...
export function AdminMercadoPage() {
  ...
  const saveProduct = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const name = String(data.get("name") ?? "").trim(); const productCategory = String(data.get("category") ?? ""); const price = Number(data.get("price") ?? 0); const stock = Number(data.get("stock") ?? 0);
    if (!name || !productCategory || price < 0 || stock < 0) { toast.error("Preencha nome, categoria, preço e estoque corretamente."); return; }
    setSaving(true);
    try { if (editing) await updateProduct(editing.id, { name, category: productCategory, price, stock }); else await createProduct({ name, category: productCategory, price, stock }); toast.success(editing ? "Produto atualizado em tempo real." : "Produto adicionado ao estoque."); setEditing(null); setCreating(false); }
