...
const ROLE_ACCESS: Record<RoleLabel, AppRole[]> = {
  "Hóspede": ["guest", "admin"],
  "Proprietário": ["host", "admin"],
  "Administrador": ["admin"],
  "Administração": ["admin"],
};
...
export function DashboardShell({ title, subtitle, role, nav, logoutTo, children }: Props) {
  ...
  const handleLogout = async () => {
    ...
    navigate({ to: logoutTo || "/", replace: true });
  };
  ...
  const NavList = ({ onNav }: { onNav?: () => void }) => (
    <nav className="flex flex-col gap-1 px-3">
      ...
      <button
        ...
        className="mt-2 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-destructive hover:bg-destructive/10 cursor-pointer"
      >
    ...
    <aside className="flex h-full w-64 flex-col border-r bg-sidebar">
      <div className="flex h-16 items-center px-5 border-b">
        <Link to="/" className="flex items-center gap-2.5 hover:opacity-90 transition-opacity">
          <img src="/zelo-logo.png" alt="Zelo Logo" className="h-8 w-auto object-contain rounded" />
          <span className="font-serif font-bold text-lg tracking-wider text-foreground">Zelo</span>
        </Link>
      </div>
      <div className="flex-1 overflow-y-auto py-4">
        <div className="px-6 pb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          {role}
        </div>
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-2 border-b bg-background/80 px-3 backdrop-blur-md sm:gap-3 sm:px-6">
          <Sheet open={open} onOpenChange={setOpen}>
            ...
            <SheetContent side="left" className="w-72 p-0">
              <SheetHeader className="sr-only">
                <SheetTitle>Menu Zelo</SheetTitle>
              </SheetHeader>
              {Sidebar}
            </SheetContent>
          </Sheet>
          ...
          >
            ...
            <PopoverContent align="end" className="w-[min(20rem,calc(100vw-2rem))] p-0">
              <div className="border-b px-4 py-3 font-semibold">Notificações</div>
              <div className="max-h-80 overflow-y-auto divide-y">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => (
                    <div key={n.id} className={cn("px-4 py-3", !n.read_at && "bg-accent/40")}>
        ...
        <footer className="border-t px-4 py-3 text-center text-[11px] text-muted-foreground sm:px-6 lg:px-8">
