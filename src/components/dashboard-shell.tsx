...
export function DashboardShell({ title, subtitle, role, nav, logoutTo, children }: Props) {
  ...
  const requestBrowserNotifications = async () => {
    try {
      ...
      setPushActive(true);
      toast.success("Push ativado neste dispositivo.", { description: "Você receberá alertas de novos pedidos e chamados mesmo fora da tela do sino." });
    } catch (error) { toast.error(error instanceof Error ? error.message : "Não foi possível ativar o push."); }
  };
  ...
  const notificationLabel = userRole === "guest" ? "Seus avisos" : userRole === "host" ? "Avisos dos seus imóveis" : "Notificações";
  ...
  return <div className="flex min-h-screen w-full bg-background"><div className="hidden lg:block">{Sidebar}</div><div className="flex min-w-0 flex-1 flex-col"><header className="sticky top-0 z-30 flex h-16 items-center gap-2 border-b bg-background/80 px-3 backdrop-blur-md sm:gap-3 sm:px-6"><Sheet open={open} onOpenChange={setOpen}><SheetTrigger asChild><Button variant="ghost" size="icon" className="shrink-0 lg:hidden"><Menu className="h-5 w-5" /></Button></SheetTrigger><SheetContent side="left" className="w-72 p-0"><SheetHeader className="sr-only"><SheetTitle>Menu Zelo</SheetTitle></SheetHeader>{Sidebar}</SheetContent></Sheet><div className="min-w-0 flex-1"><h1 className="truncate font-serif text-base font-semibold sm:text-xl">{title}</h1>{subtitle && <p className="truncate text-xs text-muted-foreground">{subtitle}</p>}</div><Popover open={notificationsOpen} onOpenChange={(o) => { setNotificationsOpen(o); if (o && unread > 0) markRead.mutate(); }}><PopoverTrigger asChild><Button variant="ghost" size="icon" className="relative shrink-0" aria-label={notificationLabel}><Bell className="h-5 w-5" />{unread > 0 && <span className="absolute right-1 top-1 grid h-4 min-w-4 place-items-center rounded-full bg-gold px-1 text-[10px] font-semibold text-gold-foreground">{unread}</span>}</Button></PopoverTrigger><PopoverContent align="end" className="w-[min(21rem,calc(100vw-2rem))] p-0"><div className="border-b px-4 py-3"><div className="font-semibold">{notificationLabel}</div><div className="mt-0.5 text-[11px] text-muted-foreground">{userRole === "guest" ? "Pedidos, pagamentos e chamados da sua estadia." : userRole === "host" ? "Chamados e movimentações dos seus imóveis." : "Movimentações operacionais da Zelo."}</div></div>{userRole === "admin" && <div className="border-b px-4 py-3"><Button variant="outline" size="sm" className="w-full gap-2" onClick={() => void requestBrowserNotifications()} disabled={pushActive}><Smartphone className="h-4 w-4" />{pushActive ? "Push ativo neste dispositivo" : browserPermission === "denied" ? "Notificações bloqueadas no navegador" : "Ativar push neste dispositivo"}</Button><p className="mt-2 text-[11px] text-muted-foreground">Autorize este celular, notebook ou computador. Cada dispositivo pode ser ativado separadamente.</p></div>}<div className="max-h-80 overflow-y-auto divide-y">{notifications.length === 0 ? <div className="px-4 py-6 text-center text-sm text-muted-foreground">Nenhuma notificação</div> : notifications.map((n) => <button key={n.id} type="button" className={cn("block w-full px-4 py-3 text-left transition-colors hover:bg-accent", !n.read_at && "bg-accent/40")} onClick={() => { setNotificationsOpen(false); if (n.link) navigate({ to: n.link }); }}><div className="text-sm font-medium">{n.title}</div>{n.body && <div className="text-xs text-muted-foreground">{n.body}</div>}<div className="mt-1 text-[11px] text-muted-foreground">{formatDateTime(n.created_at)}</div></button>)}</div></PopoverContent></Popover><div className="flex shrink-0 items-center gap-2 pl-1 sm:pl-2"><Avatar className="h-8 w-8"><AvatarFallback className="bg-primary text-xs text-primary-foreground">{userName.split(" ").slice(0, 2).map((s) => s[0]).join("")}</AvatarFallback></Avatar><div className="hidden sm:block"><div className="max-w-[12rem] truncate text-sm font-medium leading-tight">{userName}</div><div className="text-xs text-muted-foreground leading-tight">{role}</div></div><Button variant="ghost" size="sm" onClick={() => void handleLogout()} className="text-xs text-muted-foreground hover:text-destructive gap-1 px-2" title="Sair da conta"><LogOut className="h-3.5 w-3.5" /><span className="hidden md:inline">Sair</span></Button></div></header><main className="flex-1 p-4 sm:p-6 lg:p-8">{children}</main><footer className="border-t px-4 py-3 text-center text-[11px] text-muted-foreground sm:px-6 lg:px-8"><DeveloperCredit className="text-primary" /></footer></div><WhatsappButton /></div>;
}
