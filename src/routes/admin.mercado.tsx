  ...
  return <DashboardShell nav={adminNav} role="Administrador" logoutTo="/admin/login" title="Minimercado & Estoque" subtitle="Cadastre produtos e ajuste o estoque. As alterações são sincronizadas em tempo real.">
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card><CardHeader className="flex flex-row items-center justify-between pb-2"><span className="text-sm text-muted-foreground">Produtos esgotados</span><AlertTriangle className="h-4 w-4 text-amber-600" /></CardHeader><CardContent><div className="text-2xl font-bold text-amber-600">{empty}</div><p className="mt-1 text-xs text-muted-foreground">Itens com 0 unidade disponível</p></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between pb-2"><span className="text-sm text-muted-foreground">Produtos com estoque baixo</span><AlertTriangle className="h-4 w-4 text-destructive" /></CardHeader><CardContent><div className="text-2xl font-bold text-destructive">{low}</div><p className="mt-1 text-xs text-muted-foreground">Itens com até 5 unidades</p></CardContent></Card>
      </div>

      <Card className="overflow-hidden"><CardHeader className="border-b bg-primary/5"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="text-lg">Catálogo de produtos</CardTitle><CardDescription>Gerencie os itens existentes no local e mantenha as quantidades atualizadas.</CardDescription></div><Button onClick={() => setCreating(true)} className="w-full gap-2 sm:w-auto"><PackagePlus className="h-4 w-4" /> Novo produto</Button></div></CardHeader>
        <CardContent className="space-y-5 p-3 sm:p-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="relative w-full sm:max-w-xs"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar produto" className="pl-9" /></div><div className="flex w-full gap-2 overflow-x-auto pb-1 sm:w-auto">{["Todas", ...categories].map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className={`shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-medium ${category === item ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>{item}</button>)}</div></div>
