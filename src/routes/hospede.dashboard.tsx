...
export function HospedeDashboard() {
  ...
  const kitsCount = orders.filter((order) => order.categoria === "Kits").length;
  const mercadoCount = orders.filter((order) => order.categoria === "Minimercado").length;
  const openCount = orders.filter((order) => !["Entregue", "Cancelado"].includes(order.status)).length;
  ...
  return <DashboardShell nav={guestNav} role="Hóspede" logoutTo="/hospede/login" title={`Olá, ${userName.split(" ")[0]}`} subtitle="Bem-vindo(a) de volta à sua estadia.">
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle className="text-lg">Últimos pedidos</CardTitle><CardDescription>Apenas os pedidos vinculados à sua conta são exibidos aqui.</CardDescription></CardHeader><CardContent>{ordersLoading ? <p className="text-sm text-muted-foreground">Carregando pedidos…</p> : orders.length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">Você ainda não realizou pedidos nesta hospedagem.</p> : <div className="divide-y">{orders.slice(0, 5).map((order) => <div key={order.id} className="flex items-center justify-between gap-4 py-3"><div><p className="text-sm font-semibold">{order.itens}</p><p className="text-xs text-muted-foreground">{order.categoria} · {order.data}</p></div><div className="text-right"><p className="text-sm font-bold">{order.valor}</p><Badge variant="secondary" className="text-[10px]">{order.status}</Badge></div></div>)}</div>}</CardContent></Card>
    </div>
  </DashboardShell>;
}
